...
from Repositories.EmployeeRepository import hasTheEmployeeEnteredForToday, insertPresenceOfEmployee, \
	hasTheEmployeeExitedForToday
...
class FaceFrame:
	def __init__(self, vs, root, width, height, x):
		...
		try:
			...
			msg.showerror('Kujdes!', 'Nuk ka të dhëna për punonjësit!')
				...
				while not self.stopEvent.is_set():
					...
					for ((top, right, bottom, left), name) in zip(boxes, names):
						...
						if name is not "I Panjohur":
							...
							if hasTheEmployeeEnteredForToday(name) is not True or hasTheEmployeeExitedForToday(name) is True:
								insertPresenceOfEmployee(name)
								firstName = name.split(' ')
								messageToPlay ='Miresevini ' + firstName[0]
								message ='Miresevini ' + name + ', ju hyte me sukses!'
					...
					if self.panel is None:
						self.panel = tki.Label(image=image, width=self.width, height = 600)
